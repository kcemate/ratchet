...
src/
  ...
  commands/
    ...
    torque.ts           ratchet torque (the main click loop)
    status.ts           ratchet status
    log.ts              ratchet log
    tighten.ts          ratchet tighten
  core/
...
To add a new agent backend, implement this interface and register it in `commands/torque.ts`.
...
3. Wire it up in `src/commands/torque.ts` where `ShellAgent` is instantiated
...
ratchet torque --target cli-polish --clicks 7
