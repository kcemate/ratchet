...
- Background mode: `--background` flag + `ratchet improve --stop` command
...
- Score-aware click prioritization: improve targets highest-ROI categories by default
...
- `ratchet map`: interactive Cytoscape.js dependency graph with quality scores and cyberpunk theme
- `ratchet report --badge`: SVG score badge generation + README snippet
- PR comment: auto-generated score cards on improve commits
...
- `ratchet improve`: click-loop engine — scan → propose → build → test → commit
- `ratchet init`: zero-config auto-detection and `.ratchet.yml` generation
- `ratchet report --status`: show current run state and score
- `ratchet report --log`: view click history with markdown rendering
- `ratchet ship`: `--pr` flag to open GitHub PRs
