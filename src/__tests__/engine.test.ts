...
import { diffCategories, generateRecommendations, computeRunEconomics, summarizeRun } from "../core/engine.js";
...
const makeScanResult = (
  categories: ReturnType<typeof makeScanCategory>[],
  total: number,
...
describe("diffCategories", () => {
  it("computes delta between two scans", () => {
    ...
    const after = makeScanResult([makeScanCategory("Testing", 55, 100, [{ name: "Coverage", issuesFound: 5 }])], 55, 5);
  ...
  it("includes categories only in the after scan", () => {
    ...
    const after = makeScanResult([makeScanCategory("Testing", 30, 100, [{ name: "Coverage", issuesFound: 8 }])], 30, 8);
    const deltas = diffCategories(before as any, after as any);
    expect(deltas).toHaveLength(1);
...
describe("generateRecommendations", () => {
  ...
  it("recommends --plan-first when rollback rate > 30%", () => {
    const clicks: ClickEconomics[] = [click("landed", 1), click("rolled-back", 0), click("rolled-back", 0)];
  ...
  it("recommends --architect when score delta is zero despite clicks", () => {
    const clicks: ClickEconomics[] = [click("landed", 0), click("landed", 0), click("landed", 0)];
    const recs = generateRecommendations(clicks);
  ...
  it("returns empty array for healthy run", () => {
    const clicks: ClickEconomics[] = [click("landed", 1), click("landed", 2), click("landed", 3)];
...
describe("computeRunEconomics", () => {
  const click = (
    outcome: ClickEconomics["outcome"],
    wallTimeMs: number,
    cost: number,
    scoreDelta: number,
    issuesFixed: number
    ...
    clickIndex: 0,
    wallTimeMs,
    agentTimeMs: wallTimeMs / 2,
    testTimeMs: wallTimeMs / 4,
    estimatedCost: cost,
    outcome,
    rollbackReason: undefined,
    issuesFixed,
  ...
  it("computes total wall time correctly", () => {
    const clicks: ClickEconomics[] = [click("landed", 1000, 0.01, 5, 2), click("landed", 2000, 0.02, 3, 1)];
  ...
  it("computes rollback rate correctly", () => {
    const clicks: ClickEconomics[] = [click("landed", 1000, 0.01, 5, 2), click("rolled-back", 500, 0, 0, 0)];
  ...
  it("sums total cost across clicks", () => {
    const clicks: ClickEconomics[] = [click("landed", 1000, 0.01, 5, 2), click("landed", 1000, 0.02, 3, 1)];
    const result = computeRunEconomics(clicks, 2000);
    expect(result.totalCost).toBeCloseTo(0.03, 4);
  });

  it("sums scoreDelta across clicks", () => {
    const clicks: ClickEconomics[] = [click("landed", 1000, 0.01, 5, 2), click("landed", 1000, 0.01, 3, 1)];
    const result = computeRunEconomics(clicks, 2000);
...
describe("summarizeRun", () => {
  ...
  it("counts passed and failed clicks", () => {
    const run = makeRun(
      "running",
      [makeClick(true, "abc123"), makeClick(true, "def456"), makeClick(false)],
      new Date("2024-01-01")
    );
  ...
  it("extracts commit hashes from landed clicks", () => {
    const run = makeRun(
      "completed",
      [makeClick(true, "abc123"), makeClick(false), makeClick(true, "def456")],
      new Date(),
      new Date()
    );
    const summary = summarizeRun(run);
  ...
  it("includes total click count", () => {
    const run = makeRun("completed", [makeClick(true), makeClick(false), makeClick(true)], new Date(), new Date());
    const summary = summarizeRun(run);
