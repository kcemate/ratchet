  ...
  const before: ScanResult = {
    ...
    categories: [
      ...
      },
      {
        ...
        score: 80,
        max: 100,
        subcategories: [{ name: "security-cors", score: 0, max: 10, summary: "security-cors", issuesFound: 2 }],
      },
    ],
  };
  ...
  const after: ScanResult = {
    ...
    categories: [
      ...
      },
      {
        ...
        score: 60,
        max: 100,
        subcategories: [{ name: "performance-cache", score: 0, max: 10, summary: "performance-cache", issuesFound: 4 }],
      },
    ],
  };
      ...
      categories: [
        {
          ...
          score: 50,
          max: 100,
          subcategories: [{ name: "legacy-code", score: 0, max: 20, summary: "legacy-code", issuesFound: 10 }],
        },
      ],
    };

    const emptyAfter: ScanResult = {
      projectName: "test",
      total: 0,
      maxTotal: 100,
      totalIssuesFound: 0,
      ...
      categories: [
        {
          ...
          score: 75,
          max: 100,
          subcategories: [{ name: "test-basic", score: 0, max: 10, summary: "test-basic", issuesFound: 5 }],
        },
      ],
    };
...
describe("generateRecommendations", () => {
  ...
  it("returns empty array when no issues detected", () => {
    const clicks: ClickEconomics[] = [{ ...baseClick }, { ...baseClick }];
    const recs = generateRecommendations(clicks);
...
describe("computeRunEconomics", () => {
  ...
  it("calculates correct economics for mixed outcomes", () => {
    const clicks: ClickEconomics[] = [
      { ...baseClick, outcome: "landed", clickIndex: 0, agentTimeMs: 500, testTimeMs: 500, wallTimeMs: 1000 },
      { ...baseClick, outcome: "rolled-back", clickIndex: 0, agentTimeMs: 500, testTimeMs: 500, wallTimeMs: 500 },
      { ...baseClick, outcome: "timeout", clickIndex: 0, agentTimeMs: 500, testTimeMs: 500, wallTimeMs: 300 },
    ];
  ...
  it("includes recommendations in result", () => {
    ...
    expect(economics.recommendations).toContain(
      "3/4 clicks rolled back — consider --plan-first to reduce wasted iterations"
    );
  });
});
...
describe("getConsecutiveTrailingRollbacks", () => {
  ...
  it("counts consecutive rollbacks at end of array", () => {
    const clicks = [createClick(true), createClick(false), createClick(false), createClick(false)];
    expect(getConsecutiveTrailingRollbacks(clicks)).toBe(3);
  });

  it("returns 0 when last click passed", () => {
    const clicks = [createClick(false), createClick(false), createClick(true)];
  ...
  it("returns full length when all clicks rolled back", () => {
    const clicks = [createClick(false), createClick(false), createClick(false), createClick(false)];
