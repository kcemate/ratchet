...
describe("getReadySteps", () => {
  ...
  it("does not return already-completed steps", () => {
    const steps: FeatureStep[] = [{ id: 1, description: "Step 1", files: [], dependencies: [], status: "completed" }];
    const plan = makePlan(steps, [1]);
    const ready = getReadySteps(plan);
  ...
  it("does not return failed steps", () => {
    const steps: FeatureStep[] = [{ id: 1, description: "Step 1", files: [], dependencies: [], status: "failed" }];
    const plan = makePlan(steps);
    const ready = getReadySteps(plan);
    expect(ready).toHaveLength(0);
  });
  ...
  it("does not return in-progress steps", () => {
    const steps: FeatureStep[] = [{ id: 1, description: "Step 1", files: [], dependencies: [], status: "in-progress" }];
    const plan = makePlan(steps);
    const ready = getReadySteps(plan);
    expect(ready).toHaveLength(0);
  });
  ...
  it("returns nothing if all dependencies are unfulfilled", () => {
    const steps: FeatureStep[] = [{ id: 1, description: "Step 1", files: [], dependencies: [99], status: "pending" }];
    const plan = makePlan(steps);
    const ready = getReadySteps(plan);
    expect(ready).toHaveLength(0);
  });
...
describe("isPlanComplete", () => {
  ...
  it("returns false when any step is in-progress", () => {
    const plan: FeaturePlan = {
      spec: "Test",
      steps: [{ id: 1, description: "A", files: [], dependencies: [], status: "in-progress" }],
      completedSteps: [],
      filesCreated: [],
      filesModified: [],
    };
