...
describe('preflightTestCommand', () => {
  ...
  it('throws when JS dependencies are not installed (Cannot find module)', async () => {
    ...
    await expect(preflightTestCommand('npx vitest run', '/fake/cwd'))
      .rejects.toThrow('Dependencies are not installed');
  });
  ...
  it('throws when Python deps are missing (ModuleNotFoundError)', async () => {
    ...
    await expect(preflightTestCommand('pytest', '/fake/cwd'))
      .rejects.toThrow('Dependencies are not installed');
  });
  ...
  it('dep-missing error includes a fix command', async () => {
    ...
    await expect(preflightTestCommand('npx vitest run', '/fake/cwd'))
      .rejects.toThrow(/install your project dependencies/);
  });
  ...
  it('throws when test runner binary is missing (ENOENT)', async () => {
    ...
    await expect(preflightTestCommand('vitest run', '/fake/cwd'))
      .rejects.toThrow('Test runner not on PATH');
  });
