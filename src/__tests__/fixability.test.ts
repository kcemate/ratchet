...
describe('classifyFixability — effort signal', () => {
  it('effort 1 subcategory scores higher than effort 5', () => {
    ...
    const low = classifyFixability(makeTask({ subcategory: 'Empty catches', fixMode: 'torque', sweepFiles: ['src/a.ts'] }), 100);
    const high = classifyFixability(makeTask({ subcategory: 'Duplication', fixMode: 'architect', sweepFiles: ['src/a.ts'] }), 100);
...
describe('filterByFixability', () => {
  it('returns all tasks as actionable with threshold=0', () => {
    const tasks = [
      makeTask({ subcategory: 'Duplication', fixMode: 'architect', sweepFiles: Array.from({ length: 20 }, (_, i) => `${i}`) }),
  ...
  it('correctly splits tasks at threshold=0.8 (api-agent gate)', () => {
    ...
    const hard = makeTask({ subcategory: 'Duplication', fixMode: 'architect', sweepFiles: Array.from({ length: 20 }, (_, i) => `${i}`) });
