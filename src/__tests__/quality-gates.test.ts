...
describe("category gates", () => {
  it("passes when category meets threshold", () => {
    expect(evaluateGates(mockScanResult(), null, [{ categoryName: "Security", threshold: 10, max: 15 }]).passed).toBe(
      true
    );
  });
  ...
  it("is case-insensitive", () => {
    expect(evaluateGates(mockScanResult(), null, [{ categoryName: "security", threshold: 12, max: 15 }]).passed).toBe(
      false
    );
  });
  ...
  it("passes when category at max", () => {
    expect(
      evaluateGates(mockScanResult(), null, [{ categoryName: "Type Safety", threshold: 15, max: 15 }]).passed
    ).toBe(true);
  });
});
...
describe("combined gates", () => {
  it("fails when total fails but category passes", () => {
    expect(
      evaluateGates(mockScanResult({ total: 70 }), 80, [{ categoryName: "Type Safety", threshold: 15, max: 15 }]).passed
    ).toBe(false);
  });
  it("fails when category fails but total passes", () => {
    expect(
      evaluateGates(mockScanResult({ total: 90 }), 80, [{ categoryName: "Security", threshold: 12, max: 15 }]).passed
    ).toBe(false);
  });
});
