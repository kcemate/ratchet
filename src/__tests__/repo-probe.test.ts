...
import { probeRepo, getVerifyCommand, getBuildCommand, getLintCommand, getSourceRoots } from "../core/repo-probe.js";
...
describe("cache", () => {
  ...
  it("respects 24h expiry — reads stale cache as expired", () => {
    ...
    writeFileSync(join(dir, ".ratchet", "repo-profile.json"), JSON.stringify(staleProfile, null, 2));
  ...
  it("uses fresh cache within 24h window", () => {
    ...
    mkdirSync(join(dir, ".ratchet"), { recursive: true });
    writeFileSync(join(dir, ".ratchet", "repo-profile.json"), JSON.stringify(freshProfile, null, 2));
