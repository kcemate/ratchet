...
import { buildSemanticDependencies, parseLocalImports, type VisionEdge, type RiskCluster } from "../core/vision.js";
...
const VALID_SEMANTIC_RESPONSE = JSON.stringify({
  ...
  riskClusters: [{ id: "cluster-1", label: "Auth Core", files: ["src/a.ts", "src/b.ts"], reason: "Share JWT state" }],
});
...
describe("buildSemanticDependencies — semantic edges added to graph", () => {
  ...
  it("ignores edges referencing files not in the input list", async () => {
    const unknownFileResponse = JSON.stringify({
      semanticDependencies: [{ source: "src/unknown.ts", target: "src/a.ts", reason: "ghost file" }],
      riskClusters: [],
    });
...
describe("generateVisionHTML — without --deep: unchanged behavior", () => {
  const baseGraph: VisionGraph = {
    ...
    edges: [{ source: "/p/a.ts", target: "/p/b.ts", type: "import" }],
...
describe("generateVisionHTML — with --deep: semantic edge rendering", () => {
  const deepGraph: VisionGraph = {
    ...
    truncated: false,
    deepMode: true,
    riskClusters: [{ id: "rc-1", label: "Core Auth", files: ["/p/a.ts", "/p/b.ts"], reason: "Share session state" }],
  };
