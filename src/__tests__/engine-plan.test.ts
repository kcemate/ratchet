...
import type { RatchetRun, Target, PlanResult, ScanResult } from '../types.js';
...
describe('runPlanFirst', () => {
  ...
  it('should generate a plan when agent returns valid JSON', async () => {
    ...
    expect(mockAgent.runDirect).toHaveBeenCalled();
    expect(mockRun.planResult).toBeDefined();
    expect(mockRun.planResult?.steps).toEqual(['step1', 'step2']);
    expect(mockRun.planResult?.strategy).toBe('direct');
  ...
  it('should handle agent output without JSON gracefully', async () => {
    mockAgent.runDirect = vi.fn().mockResolvedValue('No valid JSON here');
  ...
  it('should handle agent errors gracefully (non-fatal)', async () => {
    mockAgent.runDirect = vi.fn().mockRejectedValue(new Error('Agent failed'));
  ...
  it('should include scan summary in prompt when scan result is provided', async () => {
    ...
    const calledWith = mockAgent.runDirect?.mock.calls[0];
    expect(calledWith).toBeDefined();
