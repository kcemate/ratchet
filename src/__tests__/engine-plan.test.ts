...
describe('runPlanFirst', () => {
  ...
  beforeEach(() => {
    ...
    mockAgent = {
      analyze: vi.fn(async () => ''), propose: vi.fn(async () => ''), build: vi.fn(async () => ({ success: true, output: '' })) as any, runDirect: vi.fn().mockResolvedValue('{"steps": ["step1", "step2"], "strategy": "direct"}'),
    } as any;
  ...
  it('should handle agent output with markdown code fences', async () => {
    (mockAgent as any).runDirect = vi.fn().mockResolvedValue('```json\n{"steps": ["step1", "step2"], "strategy": "direct"}\n```');
