...
function mockSimple(stdout: string, stderr = "", exitCode = 0) {
  mockForArgs([{ match: () => true, stdout, stderr, exitCode }]);
}
...
describe("checkoutBranch", () => {
  it("checks out a named branch", async () => {
    ...
    expect(mockExecFile).toHaveBeenCalledWith("git", ["checkout", "main"], expect.any(Object), expect.any(Function));
  });
});
...
describe("stashPop", () => {
  it("runs git stash pop", async () => {
    mockSimple("");
    await stashPop(CWD);
    expect(mockExecFile).toHaveBeenCalledWith("git", ["stash", "pop"], expect.any(Object), expect.any(Function));
  });
});
...
describe("commit", () => {
  it("stages, commits, and returns the new hash", async () => {
    ...
    mockExecFile.mockImplementation((_cmd: string, _args: string[], _opts: unknown, cb: Function) => {
      step++;
      if (step === 1)
        cb(null, { stdout: "", stderr: "" }); // add
      else if (step === 2)
        cb(null, { stdout: "", stderr: "" }); // commit
  ...
  it("throws with a clear message when nothing to commit", async () => {
    ...
    mockExecFile.mockImplementation((_cmd: string, _args: string[], _opts: unknown, cb: Function) => {
      ...
      if (step === 1)
        ...
        cb(Object.assign(new Error("nothing"), { stderr: "nothing to commit" }), {
          stdout: "",
...
describe("gitDropStash", () => {
  it("drops the stash", async () => {
    ...
    expect(mockExecFile).toHaveBeenCalledWith("git", ["stash", "drop"], expect.any(Object), expect.any(Function));
  });
});
