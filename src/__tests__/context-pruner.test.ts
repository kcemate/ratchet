...
describe("buildClickContext", () => {
  it("includes issue category and subcategory in output", () => {
    const scan = makeScan();
    const issues: IssueTask[] = [makeTask({ category: "Code Quality", subcategory: "console-log", count: 3 })];
  ...
  it("maps sweep files to relevance 1.0", () => {
    const scan = makeScan();
    const issues: IssueTask[] = [makeTask({ sweepFiles: ["src/alpha.ts", "src/beta.ts"] })];
  ...
  it("excludes files not referenced by target issues", () => {
    const scan = makeScan({
      issuesByType: [makeIssue({ subcategory: "any-type", locations: ["src/unrelated.ts"] })],
    });
    const issues: IssueTask[] = [makeTask({ subcategory: "console-log", sweepFiles: ["src/relevant.ts"] })];
    const { fileRelevanceMap } = buildClickContext(scan, issues, "/cwd");
  ...
  it("picks up locations from scan issuesByType when subcategory matches", () => {
    ...
    const issues: IssueTask[] = [makeTask({ subcategory: "console-log", sweepFiles: [] })];
    const { fileRelevanceMap } = buildClickContext(scan, issues, "/cwd");
