...
describe('path helpers', () => {
  ...
  it('agentClaimPath builds correct path', () => {
    expect(agentClaimPath('/cwd', 'run1', 'agent-1')).toBe(
      '/cwd/.ratchet/parallel/run1/claims/agent-agent-1.json',
    );
  });
});
...
describe('claimFiles', () => {
  ...
  it('overwrites an existing claim for the same agent', () => {
    ...
    const payload = JSON.parse(
      readFileSync(agentClaimPath(tmpCwd, RUN_ID, 'agent-a'), 'utf-8'),
    ) as ClaimPayload;
...
describe('writeDiscovery', () => {
  ...
  it('writes a discovery JSON file', () => {
    ...
    const content = JSON.parse(
      readFileSync(join(discoveriesDir(tmpCwd, RUN_ID), files[0]!), 'utf-8'),
    ) as Discovery;
...
describe('readDiscoveries', () => {
  ...
  it('ignores corrupt discovery files', () => {
    ...
    writeFileSync(
      join(discoveriesDir(tmpCwd, RUN_ID), 'discovery-agent-b-12345.json'),
      'CORRUPT',
      'utf-8',
    );
    expect(readDiscoveries(tmpCwd, RUN_ID, 'agent-a')).toEqual([]);
