...
import { nodeColor, glowColor, computeFileScore, parseLocalImports, getNeighborhood } from "../../src/core/vision.js";
...
describe("parseLocalImports", () => {
  const allFiles = new Set(["/project/src/core/utils.ts", "/project/src/core/engine.ts", "/project/src/lib/cli.ts"]);
  ...
  it("deduplicates repeated imports of the same file", () => {
    const content = [`import { a } from './utils.js';`, `import { b } from './utils.js';`].join("\n");
    const result = parseLocalImports(content, "/project/src/core/main.ts", allFiles);
