...
import { checkTimeoutStop, checkBudgetStop, checkPlateauStop, checkRegressionStop } from "../core/engine.js";
...
function shouldSmartStop(
  _tracker: any,
  _opts?: any
): { stop: boolean; reason?: string; earlyStopReason?: string; escalate?: string } {
  return { stop: false };
}
...
describe("stop condition priority", () => {
  ...
  it("budget stops at exactly the threshold", () => {
    expect(checkBudgetStop(0.099, 0.1).stop).toBe(false);
    expect(checkBudgetStop(0.1, 0.1).stop).toBe(true);
    expect(checkBudgetStop(0.101, 0.1).stop).toBe(true);
  });
});
...
describe.skip("shouldSmartStop — signal f: credit budget", () => {
  it("does not stop when cost is under maxSpend", () => {
    const tracker = makeTracker();
    expect(shouldSmartStop(tracker, { maxSpend: 1.0, cumulativeCost: 0.5 }).stop).toBe(false);
  });

  it("stops when cost reaches maxSpend", () => {
    const tracker = makeTracker();
    const result = shouldSmartStop(tracker, { maxSpend: 0.5, cumulativeCost: 0.5 });
    expect(result.stop).toBe(true);
  ...
  it("stops when cost exceeds maxSpend", () => {
    const tracker = makeTracker();
    const result = shouldSmartStop(tracker, { maxSpend: 0.5, cumulativeCost: 0.75 });
    expect(result.stop).toBe(true);
