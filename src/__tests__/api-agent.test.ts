...
describe("APIAgent.build() — guard limits", () => {
  ...
  it("reads up to maxFiles when clickGuards.maxFiles > 1", async () => {
    ...
    const capturedPrompt = await capturePrompt(tempDir, `Path: src/a.ts\nPath: src/b.ts\nFix the issue.`, {
      maxFiles: 2,
      maxLines: 50,
    });
    const editableSection = capturedPrompt.split("READ-ONLY CONTEXT")[0];
  ...
  it("prompt contains correct maxFiles/maxLines from guards", async () => {
    const capturedPrompt = await capturePrompt(tempDir, "Fix the issue.", { maxFiles: 3, maxLines: 60 });
...
describe("APIAgent.build() — read-only context injection", () => {
  it("injects test file as READ-ONLY CONTEXT when found", async () => {
    ...
    const capturedPrompt = await capturePrompt(tempDir, `Path: src/utils.ts\nFix the function.`, {
      maxFiles: 1,
      maxLines: 20,
    });
  ...
  it("injects imported file as READ-ONLY CONTEXT", async () => {
    ...
    await writeFile(join(tempDir, "src/main.ts"), `import { helper } from './helpers';\nexport const x = helper;`);

    const capturedPrompt = await capturePrompt(tempDir, `Path: src/main.ts\nFix the import.`, {
      maxFiles: 1,
      maxLines: 20,
    });
    expect(capturedPrompt).toContain("READ-ONLY CONTEXT");
  ...
  it("does not include read-only context when no related files exist", async () => {
    ...
    const capturedPrompt = await capturePrompt(tempDir, `Path: src/standalone.ts\nFix the value.`, {
      maxFiles: 1,
      maxLines: 20,
    });
