...
export interface RatchetConfig {
  ...
  models?: ModelTiers;
  /** Provider routing config (api keys via env vars, base URLs, model lists) */
  ...
  scan?: {
     ...
     *   classic  fast deterministic heuristics (default, free)
     ...
     * Also overridable via RATCHET_SCAN_MODEL env var or --scan-model CLI flag.
     */
    model?: string;
  };
...
export interface RatchetRun {
  ...
  sweepEscalated?: boolean; // set when the engine escalated to sweep mode mid-run
  sweepEscalatedAtClick?: number; // the click number at which sweep escalation happened
  sweepEscalatedCategory?: string; // the subcategory that triggered sweep escalation
