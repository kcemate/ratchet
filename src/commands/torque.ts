    ...
    for (const sub of cat.subcategories) {
      ...
      if (sub.locations && sub.locations.length > 0) {
        ...
        if (sub.locations.length > 5) {
          process.stdout.write(chalk.dim(`    ${"".padEnd(22)}  ... and ${sub.locations.length - 5} more\n`));
        }
      } else {
        process.stdout.write(
...
export function torqueCommand(): Command {
  ...
  cmd
    ...
    .option("--dry-run", "Preview mode — analyze and propose without committing any changes (default: true)", true)
    ...
    .action(
      async (options: {
        ...
        if (options.dryRun && !options.apply) {
          pinoLogger.info(chalk.yellow("  ⚠  Dry-run mode (default). Use --apply to commit changes."));
        }
        ...
        let clickCount = options.clicks ? parseInt(options.clicks, 10) : options.sweep ? 5 : config.defaults.clicks;
        ...
        try {
          if (options.deep && engineFn === runEngine && !options.dryRun) {
            ...
            while (remaining > 0) {
              ...
              if (segRuns.length > 0) {
                const reDeepSpinner = ora(`  Re-running Deep analysis (click ${deepClickOffset + 1})…`).start();
                try {
            ...
            run = await engineFn({
              ...
              callbacks: {
                onScanComplete: (scan: ScanResult) => {
                  ...
                  const targetStr = topIssues.map(t => `${t.subcategory} (${t.count}/${t.count + 1})`).join(", ");
                  process.stdout.write(
                  ...
                  if (!options.architect) {
                    ...
                    const structuralDominant =
                      structuralPoints >= reachablePoints || (nonSweepableGaps.length > 0 && reachablePoints <= 2);
                ...
                onClickStart: async (clickNumber, total, hardenPhase?: HardenPhase) => {
                  ...
                  currentHardenPhase = hardenPhase;
                  const phaseTag = hardenPhase ? chalk.dim(` [${hardenPhase}]`) : "";
                  spinner = ora(`  Click ${chalk.bold(String(clickNumber))}/${total}${phaseTag} — analyzing…`).start();
                ...
                onClickComplete: async (click: Click, rolledBack: boolean) => {
                  ...
                  if (!options.pro && (rolledBack || (clickDelta !== undefined && clickDelta <= 0))) {
                    process.stdout.write(chalk.dim("  ⚡ Tip: Use --pro for the best model tier\n"));
                  }
                  ...
                  if (options.verbose) {
                    ...
                    if (click.proposal) {
                      const preview = click.proposal.length > 120 ? click.proposal.slice(0, 120) + "…" : click.proposal;
                      process.stdout.write(chalk.dim(`     proposal: ${preview}`) + "\n");
                    }
                    if (click.filesModified.length > 0) {
                      process.stdout.write(chalk.dim(`     files: ${click.filesModified.join(", ")}`) + "\n");
                    }
                  }
                },
        ...
        if (options.deep && deepTotalCost > 0) {
          process.stdout.write(chalk.dim(`  Deep analysis cost: $${deepTotalCost.toFixed(2)} total\n`));
        }
        ...
        const rolledPart = rolledBack > 0 ? ` · ${chalk.yellow(String(rolledBack))} rolled back` : "";
        ...
        process.stdout.write("\n" + chalk.bold("  " + "─".repeat(46)) + "\n");
        process.stdout.write(
          `\n  ${chalk.bold("Done.")} ` + `${landedPart}${rolledPart} · ` + `${chalk.dim(duration)}\n`
        );
        ...
        if (passedClicks > 0) {
          ...
          if (reportPath) {
            process.stdout.write(`  Report: ${chalk.dim(`docs/${target.name}-ratchet-report.md`)}\n`);
          }
          process.stdout.write(`  Run ${chalk.green("ratchet ship --pr")} to open a pull request.\n` + "\n");
        } else {
          ...
          if (fps > 0 || skipped > 0) {
            ...
            if (fps > 0) {
              process.stdout.write(
                chalk.dim(`  • ${fps} false positive(s) filtered (patterns found only in comments, strings, or docs)\n`)
              );
            }
            if (skipped > 0) {
              process.stdout.write(
