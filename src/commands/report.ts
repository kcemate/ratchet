...
export function reportCommand(): Command {
  ...
  cmd
    ...
    .option(
      "--format <type>",
      'Output format: "pdf", "markdown", "json", or "both" (default: both = markdown + pdf)',
      "both"
    )
    ...
    .action(
      async (opts: {
        ...
        const scoreBefore: ScanResult | undefined = entry.scoreBefore;
        ...
        if (opts.deep) {
          try {
            ...
            process.stdout.write(
              `  ${chalk.green("✓")} Deep analysis complete — ${deepResult.issuesByType.length} issue types found\n`
            );
          } catch (err) {
