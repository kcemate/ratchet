...
export async function classifyRepo(cwd: string): Promise<RepoClassification> {
  ...
  const excludeCandidates = [
    ...
    "e2e",
    "integration-tests",
    "integration_tests", // e2e / integration
...
export function buildConfig(project: DetectedProject, targetDir: string): string {
  ...
  const frameworkNote = project.testFramework ? `  # Detected test framework: ${project.testFramework}\n` : "";
...
export function initCommand(): Command {
  ...
  cmd
    ...
    .action(async (dir: string, options: { force: boolean }) => {
      ...
      if (await exists(configPath)) {
        if (!options.force) {
          process.stdout.write(
            chalk.yellow("  .ratchet.yml already exists.") + chalk.dim(" Use --force to overwrite.") + "\n"
          );
          process.exit(1);
        }
      ...
      try {
        [project, classification] = await Promise.all([detectProject(cwd), classifyRepo(cwd)]);
