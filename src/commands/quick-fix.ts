...
export function quickFixCommand(): Command {
  ...
  cmd
    ...
    .action(async (dir: string, options: Record<string, unknown>) => {
      ...
      if (options["apply"]) {
        process.stdout.write(chalk.cyan("Coming soon — use ratchet torque --focus <category> to fix automatically\n"));
        return;
      }
      ...
      for (const item of top3) {
        ...
        if (item.issuesFound > 0) {
          process.stdout.write(`     ${item.issuesFound} ${item.issuesDescription}\n`);
        }

        process.stdout.write(`     ${chalk.dim("Fix:")} ${item.fix}\n`);
        process.stdout.write(`     ${chalk.dim("→")} Fixing this would bring your score to ${projectedStr}\n`);
        process.stdout.write("\n");
      }
      ...
      process.stdout.write("\n");
      process.stdout.write(chalk.dim("Run 'npx ratchet torque --focus <category>' to fix automatically.\n"));
      process.stdout.write("\n");
    });
