...
function printBackgroundRuns(runs: ProgressState[]): void {
  ...
  for (const r of active) {
    ...
    const clickStr = r.clicksTotal > 0 ? `${r.clicksCompleted}/${r.clicksTotal} clicks` : `${r.clicksCompleted} clicks`;
    const scoreStr = r.score !== undefined ? ` · score ${r.score}` : "";
    const pidStr = alive ? chalk.dim(`PID ${r.pid}`) : chalk.red(`PID ${r.pid} (dead)`);
    log(`    ${chalk.cyan(r.runId.slice(0, 8))}… · ${clickStr}${scoreStr} · ${chalk.yellow(elapsed)} · ${pidStr}`);
...
export function statusCommand(): Command {
  ...
  cmd
    ...
    .action(async (options: { follow?: string }) => {
      ...
      log(`  Target  : ${chalk.cyan(run.target.name)} ${chalk.dim(`(${run.target.path})`)}`);
      if (run.target.description) {
        log(`  Desc    : ${chalk.dim(run.target.description)}`);
      }
      log(`  Status  : ${colorStatus(run.status, staleRunning)}`);
      log(`  Clicks  : ${chalk.green(String(passedClicks))} passed / ${totalClicks} total`);
      ...
      if (run.status === "completed" && passedClicks > 0) {
        log("\n  " + chalk.dim("Run ") + chalk.cyan("ratchet tighten --pr") + chalk.dim(" to create a pull request."));
      }
