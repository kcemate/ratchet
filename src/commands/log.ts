...
function renderInlineCode(line: string): string {
  return line.replace(/`([^`]+)`/g, (_match, code) => chalk.cyan(code));
}
...
function renderMarkdown(content: string): void {
  ...
  for (const line of lines) {
    if (line.startsWith('# ')) {
      console.log(chalk.bold.white(renderInlineCode(line)));
    } else if (line.startsWith('## ')) {
      // Colorize click status indicators
      if (line.includes('✅')) {
        console.log('\n' + chalk.bold.green(renderInlineCode(line)));
      } else if (line.includes('❌')) {
        console.log('\n' + chalk.bold.red(renderInlineCode(line)));
      } else {
        console.log('\n' + chalk.bold.cyan(renderInlineCode(line)));
      }
    } else if (line.startsWith('### ')) {
      console.log(chalk.bold(renderInlineCode(line)));
    } else if (line.startsWith('> ')) {
      console.log(chalk.italic.dim(renderInlineCode(line)));
    } else if (line.startsWith('| ')) {
      ...
      console.log(renderInlineCode(line));
    }
  }
}
