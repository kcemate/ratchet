...
export function scoreTestQuality(
  ...
  if (testCaseCount >= 100 && assertsPerTest >= 1.75 && hasDescribe)
    return { score: 8, summary: `${assertsPerTest.toFixed(1)} assertions per test across large suite` };
...
export function scoreAuthChecks(
  ...
  if (checks >= 3) {
    score = 6;
    summary = "auth middleware, rate limiting, CORS configured";
  } else if (checks === 2) {
    score = 4;
    const found: string[] = [];
  ...
  if (broadMiddlewareCount > 0) {
    ...
    summary +=
      ` (${broadMiddlewareCount} overly broad rate limiter${broadMiddlewareCount > 1 ? "s" : ""}` +
...
export function scoreAnyTypeDensity(anyCount: number, totalLines: number): { score: number; summary: string } {
  ...
  if (anyCount === 0 || density < 1) {
    const summary =
      anyCount === 0 ? "zero any types" : `${anyCount} any type${anyCount !== 1 ? "s" : ""} (very low density)`;
...
// ── Issue aggregation
export function aggregateAndSortIssues(categories: CategoryResult[]): {
