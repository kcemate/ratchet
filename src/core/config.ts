...
function parseProviders(raw: RawConfig["providers"]): Record<string, ProviderConfig> | undefined {
...
export function getConfigWarnings(raw: string): string[] {
  ...
  if (knownModels.size > 0) {
    const modelEntries: Array<[string, string | undefined]> = [
      ["model", data.model],
      ...(data.models
        ? Object.entries(data.models).map(([k, v]) => [`models.${k}`, v] as [string, string | undefined])
        : []),
    ];
    for (const [field, value] of modelEntries) {
      if (value && !knownModels.has(value)) {
        warnings.push(`Unknown model "${value}" in ${field} — not listed in any provider's models array.`);
      }
    }
  }
