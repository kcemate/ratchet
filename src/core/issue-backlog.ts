...
export function buildBacklog(scan: ScanResult): IssueTask[] {
  ...
  for (const issue of scan.issuesByType) {
    ...
    // Look up tier definition for fixMode, fixInstruction, and currentScore
    const tierDef = SUBCATEGORY_TIERS.find(t => t.name === issue.subcategory);
    const fixMode = tierDef?.fixMode;
...
/**
 * Filter a backlog to only issues that can be addressed in the given mode.
 *
 * - 'torque': only torque-mode issues (single-file fixes)
 * - 'sweep': torque + sweep issues (batch mechanical fixes)
 * - 'architect': all issues (no filtering)
 *
