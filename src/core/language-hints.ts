...
const FIX_OVERRIDES: Record<SupportedLanguage, Record<string, string>> = {
  ...
  python: {
    'Strict config': 'Configure mypy or pyright for static type checking. Add [tool.mypy] to pyproject.toml or' +
  ...
  go: {
    ...
    'Any type count': 'Replace interface{} and any with specific types. Use generics (Go 1.18+) for' +
  ...
  rust: {
    ...
    'Any type count': 'Use concrete types instead of Box<dyn Trait> where possible.' +
      ' Apply generics for zero-cost abstractions.',
    'Coverage': 'Return Result<T, E> and propagate errors with the ? operator instead of .unwrap() or .expect().',
    'Empty catches': 'Replace .unwrap() / .expect() with proper error handling:' +
  ...
  java: {
    'Strict config': 'Configure maven-compiler-plugin with source/target versions. Enable -Xlint:all and' +
      ' use a static analysis tool like SpotBugs or Checkstyle.',
    'Any type count': 'Replace raw types (List, Map) with generic equivalents (List<String>, Map<String, Object>).' +
    ...
    'Async patterns': 'Use CompletableFuture.exceptionally() or handle() to handle async errors.' +
      ' Avoid fire-and-forget.',
    'Console cleanup': 'Remove System.out.println / e.printStackTrace() debug calls and replace with SLF4J logger' +
      ' calls.',
  },

  kotlin: {
    'Strict config': 'Enable strict Kotlin compiler options.' +
      ' Add -Werror to treat warnings as errors in build.gradle.kts.',
    'Any type count': 'Replace Any? with specific nullable types.' +
      ' Use sealed classes or Result<T> for typed error handling.',
    'Coverage': 'Wrap code in try/catch blocks. Use runCatching { } for concise exception handling.',
    'Empty catches': 'Replace empty catch blocks with logging or rethrowing:' +
      ' catch (e: Exception) { log.error("msg", e) }.',
    'Structured logging': 'Use kotlin-logging (KotlinLogging.logger { }) or SLF4J for structured logging instead of' +
      ' println.',
    'Async patterns': 'Handle coroutine exceptions with CoroutineExceptionHandler or supervisorScope.' +
  ...
  csharp: {
    ...
    'Structured logging': 'Use Microsoft.Extensions.Logging or Serilog for structured logging' +
      ' instead of Console.WriteLine.',
    'Async patterns': 'Use async Task / async ValueTask and await all async calls.' +
      ' Avoid .Result or .Wait() which can deadlock.',
    'Console cleanup': 'Remove Console.WriteLine / Debug.WriteLine debug calls' +
      ' and replace with _logger.LogDebug() calls.',
  },

  php: {
    'Strict config': 'Configure PHPStan (phpstan.neon) or Psalm (psalm.xml) for static analysis.' +
      ' Add declare(strict_types=1).',
    'Any type count': 'Add full type hints (parameter types, return types) to all functions.' +
    ...
    'Structured logging': 'Use Monolog or PSR-3 Logger interface for structured logging' +
      ' instead of var_dump/print_r/dd.',
    'Async patterns': 'Handle exceptions in async/reactive code (ReactPHP, Amphp).' +
      ' Use proper promise rejection handlers.',
    'Console cleanup': 'Remove var_dump(), print_r(), and dd() debug calls.' +
      ' Use Log::debug() or logger()->debug() instead.',
  },
};
