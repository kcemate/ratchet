...
function toAbsolute(files: string[], cwd: string): string[] {
  return files.filter(Boolean).map(f => normalizePath(isAbsolute(f) ? f : resolve(cwd, f)));
}
...
export function validateScope(modifiedFiles: string[], scopeFiles: string[], cwd: string): ScopeValidation {
...
export function formatScopeForDisplay(scopeArg: string | undefined, scopeFiles: string[], cwd: string): string {
