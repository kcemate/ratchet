...
export const CODE_EXTENSIONS = new Set(['.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.py', '.go', '.rs', '.java', '.kt', '.kts', '.cs', '.php']);
...
export function scorePythonTypeConfig(cwd: string): { score: number; summary: string } {
  if (existsSync(join(cwd, 'pyrightconfig.json')))
    return { score: 7, summary: 'pyright configured' };
  if (existsSync(join(cwd, 'mypy.ini')))
    return { score: 7, summary: 'mypy configured' };
...
export function scoreJavaTypeConfig(cwd: string): { score: number; summary: string } {
  ...
  if (existsSync(join(cwd, 'lombok.config')))
    return { score: 5, summary: 'Lombok configured' };
  ...
  if (existsSync(gradlePath)) {
    try {
      ...
      if (content.includes('kotlinOptions'))
        return { score: 4, summary: 'Kotlin compiler options in build.gradle' };
    } catch { /* ignore */ }
  }
