...
import { astConfirmedCount } from "./classic-file-analysis.js";
...
export function scoreTests(
  ...
  const effectiveTestFiles = lang === "rust" ? [...new Set(rustTestFiles)] : testFiles;
  ...
  const REACT_COMPONENT_PATTERN =
    /export\s+default\s+function\s+\w+|export\s+const\s+\w+\s*(?:=\s*React\.FC|:\s*React\.FC)/;
  let weightedSourceCount = 0;
  for (const f of sourceFiles) {
    const norm = f.replace(/\\/g, "/");
    const isUiComponent = UI_PATH_PATTERN.test(norm) && REACT_COMPONENT_PATTERN.test(contents.get(f) ?? "");
  ...
  if (lang === "ts" || lang === "js") {
    ...
    hasDescribe = describePattern !== null ? countMatches(effectiveTestFiles, contents, describePattern) > 0 : false;
  }
  ...
  return {
    ...
    subcategories: [
      ...
      },
      {
        ...
        issuesFound: edgeCaseCount === 0 ? 1 : 0,
...
export function scoreSecurity(files: string[], contents: Map<string, string>, lang: SupportedLanguage): CategoryResult {
  ...
  const customValidatorFiles = new Set<string>(
    lang === "ts" || lang === "js"
      ? srcFiles.filter(f =>
          /(?:lib|utils|helpers)\/(?:validate|validator|validation)[^/]*\.[jt]sx?$/.test(f.replace(/\\/g, "/"))
        )
      : []
  );
...
export function scoreTypes(
  ...
  if (lang === "go") {
    ...
    const { count: interfaceAnyCount } = countMatchesWithFiles(goSrcFiles, contents, /\binterface\s*\{\s*\}|\bany\b/g);
    const { locations: anyTypeFiles } = countMatchesWithLocations(
      goSrcFiles,
      contents,
      /\binterface\s*\{\s*\}|\bany\b/g
    );
    let totalLines = 0;
      ...
      subcategories: [
        {
          ...
          issuesDescription: "missing Python type checker configuration",
        },
        {
          name: "Any type count",
          score: anyScore,
          max: 8,
          summary: anySummary,
          issuesFound: anyCount,
          issuesDescription: "Any type usage",
          locations: anyTypeFiles,
        },
      ],
    };
  ...
  const { count: anyCountRegex } = countMatchesWithFiles(srcTsFiles, contents, /:\s*any\b|<any>|\bas\s+any\b/g);
  const { locations: anyTypeFiles } = countMatchesWithLocations(srcTsFiles, contents, /:\s*any\b|<any>|\bas\s+any\b/g);
...
export function scoreErrorHandling(
  ...
  if (lang === "python") {
    ...
    const { count: emptyCatchTotal, matchedFiles: emptyCatchFiles } = countMatchesWithFiles(
      prodFiles,
      contents,
      LANG_EMPTY_CATCH_PATTERNS.python
    );
    ...
    return {
      ...
      subcategories: [
        ...
        {
          name: "Empty catches",
          score: emptyCatchScore,
          max: 5,
          summary: emptyCatchSummary,
          issuesFound: emptyCatchTotal,
          issuesDescription: "bare except clauses",
        ...
        },
        {
          ...
          issuesFound: effectiveStructuredLog === 0 ? 1 : 0,
    ...
    return {
      ...
      subcategories: [
        ...
        },
        {
          ...
          max: 5,
          summary: emptyCatchSummary,
          issuesFound: ignoredErrTotal,
          issuesDescription: "ignored errors (_ = func())",
  ...
  if (lang === "rust") {
    ...
    const { count: unwrapCount, matchedFiles: unwrapFiles } = countMatchesWithFiles(
      prodFiles,
      contents,
      LANG_EMPTY_CATCH_PATTERNS.rust
    );

    const hasResultUsage = resultUsage > 0 || questionOpUsage > 0;
    const rawCoverageScore = hasResultUsage ? Math.max(5, 8 - Math.floor(unwrapCount / 5)) : 2;
    ...
    return {
      ...
      subcategories: [
        {
          name: "Coverage",
          score: Math.min(rawCoverageScore, 8),
          max: 8,
          summary: coverageSummary,
          issuesFound: unwrapCount,
          issuesDescription: "unwrap()/expect() calls that can panic",
        },
        {
          name: "Empty catches",
          score: emptyCatchScore,
          ...
          summary: emptyCatchSummary,
          issuesFound: unwrapCount,
          issuesDescription: "unwrap()/expect() (panic on error)",
  ...
  const { count: emptyCatchRegex, locations: emptyCatchLocations } = countMatchesWithLocations(
    prodFiles,
    contents,
    /\bcatch\s*(?:\([^)]*\))?\s*\{\s*\}/g
  );
  ...
  const { matchedFiles: consoleErrorFiles, count: consoleErrorRegex } = countMatchesWithFiles(
    prodAppFilesForLogging,
    contents,
    /\bconsole\.(?:error|warn|log)\s*\(/g
  );
  ...
  const customLoggerFileExportsLogger =
    hasCustomLoggerFile &&
    (() => {
      const content = contents.get(customLoggerFile!) ?? "";
      return (
        /export\s+(?:const|function|default)\s+\w*[Ll]og(?:ger)?|\bexport\s+\{.*\blog(?:ger)?\b/.test(content) &&
  ...
  const customLoggerImportCount = hasCustomLoggerFile
    ? countMatches(prodAppFilesForLogging, contents, /from\s+['"][^'"]*(?:lib|utils|helpers)\/(?:logger|log)['"]/g)
    : 0;
  ...
  const structuredLogCount =
    countMatches(prodAppFilesForLogging, contents, /\b(?:logger|winston|pino|bunyan|log4js)\./g) +
    customLoggerCallCount +
    customLoggerImportCount +
    customLoggerBonus;
  ...
  return {
    ...
    subcategories: [
      ...
      },
      {
        ...
        max: 7,
        summary: loggingSummary,
        issuesFound: structuredLogCount === 0 ? 1 : 0,
        issuesDescription: "no structured logger",
      },
    ],
  };
...
export function scorePerformance(
  ...
  if (lang === "ts" || lang === "js") {
    ...
    const hasProperLogging = guardPattern !== null ? anyFileHasMatch(prodAppFiles, contents, guardPattern) : false;
    if (hasProperLogging) {
      consoleLogCount = 0;
      consoleLogLocations = [];
    } else {
      const { count, locations } = countMatchesWithLocations(prodAppFiles, contents, LANG_DEBUG_OUTPUT_PATTERNS[lang]);
  ...
  return {
    ...
    subcategories: [
      {
        name: "Async patterns",
        score: Math.min(asyncScore, 3),
        max: 3,
        summary: asyncSummary,
        issuesFound: awaitInLoopCount,
      ...
      },
      {
        ...
        issuesFound: consoleLogCount,
        issuesDescription: debugOutputLabel,
      ...
      },
      {
        ...
        max: 2,
        summary: importHygieneSummary,
        issuesFound: importIssues,
...
// ├─ Dead code .............. /4
// └─ Duplication ............ /3
export function scoreCodeQuality(files: string[], contents: Map<string, string>): CategoryResult {
  ...
  for (const file of srcFiles) {
    const content = contents.get(file) ?? "";
    const lines = content.split("\n");
    const codeOnlyLines = stripCommentsAndStrings(content).split("\n");

    for (let li = 0; li < lines.length; li++) {
      const rawLine = lines[li] ?? "";
      const codeOnlyLine = codeOnlyLines[li] ?? "";
      if (rawLine.length > 120 && codeOnlyLine.trim().length > 120) {
        longLineCount++;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const stripped = (codeOnlyLines[i] ?? "").trim();
      const isBoilerplateLine =
        stripped.length < 25 ||
        /^[{}()[\],;:.]+$/.test(stripped) ||
        /^(?:import|export)\s/.test(stripped) ||
        /^(?:break|continue|return|throw);?$/.test(stripped);
    ...
    subcategories: [
      {
        ...
        issuesFound: longFnCount,
        issuesDescription: "functions >50 lines or high complexity",
      ...
      },
      {
        ...
        issuesFound: longLineCount,
        issuesDescription: "line >120 chars",
