...
function extractInsights(
  ...
  if (reducingLanded.length >= 1) {
    ...
    const frequencyBonus = (reducingLanded.length / Math.max(total, 1)) * 0.2;
    const sizeBonus = Math.min(0.15, avgReduced / 500);
...
function generateHotSpotNote(rollbackRate: number, attempts: number): string {
  if (rollbackRate >= 0.8) {
    return `Very hard to change — ${Math.round(rollbackRate * 100)}% rollback. Consider skipping.`;
  }
