...
export function extractDeepReportFindings(result: ScanResult): DeepReportFinding[] {
  return result.issuesByType
    ...
    .map(issue => {
      const confidence = issue.severity === "high" ? 0.9 : issue.severity === "medium" ? 0.75 : 0.6;
      return {
...
export function generateReport(options: ReportOptions): string {
  ...
  if (rolledBack.length === 0) {
    ...
    for (const click of rolledBack) {
      const reason = click.analysis ? (click.analysis.split(/[.!\n]/)[0]?.trim() ?? "Tests failed") : "Tests failed";
  ...
  if (scoreBefore && scoreAfter) {
    ...
    lines.push(`|---|---|---|---|`);
    lines.push(`| ${icon} **Production Readiness Score** | ${beforePct}/100 | ${afterPct}/100 | **${deltaStr}** |`);
