...
export async function executeParallelTask(
  ...
  const ipcRunId = runId ?? generateRunId();

  // Claim the target area before starting work
  try {
    claimFiles(mainCwd, ipcRunId, agentId, [task.target ?? task.id], task.mode);
  } catch {
    // Non-fatal — IPC is best-effort, don't block execution
  }
