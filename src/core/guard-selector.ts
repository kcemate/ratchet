...
export function classifyRisk(task: IssueTask): 'low' | 'medium' | 'high' {
  ...
  const touchesConfig = files.some((f) =>
    HIGH_RISK_CONFIG_PATTERNS.some((pattern) => f.toLowerCase().includes(pattern)),
  );
  if (touchesConfig) {
    return 'high';
  }
  ...
  const allTestFiles = files.length > 0 && files.every(
    (f) => f.includes('.test.') || f.includes('.spec.') || f.includes('__tests__') || f.includes('/test/'),
  );
  if (allTestFiles) {
    return 'low';
  }
