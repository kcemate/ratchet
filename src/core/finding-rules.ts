...
export function getRuleBySubcategory(category: string, subcategory: string): RuleDefinition | undefined {
  return _subkeyToRule.get(`${category}::${subcategory}`);
}

/** All unique categories in declaration order (deduped). */
export const RULE_CATEGORIES: string[] = [...new Set(Object.values(RULE_REGISTRY).map(r => r.category))];
