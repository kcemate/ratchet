...
export interface SubcategoryTiers {
   ...
   * - 'torque': normal single-file fix within ~30 lines
...
/**
 * Returns true if the given subcategory is marked sweepable in SUBCATEGORY_TIERS.
 * Case-insensitive match.
 */
export function isSweepable(subcategory: string): boolean {
  const lower = subcategory.toLowerCase();
  const tier = SUBCATEGORY_TIERS.find(t => t.name.toLowerCase() === lower);
  return tier?.sweepable ?? false;
}
...
export function buildScoreOptimizedBacklog(scan: ScanResult, focusCategory?: string): IssueTask[] {
  ...
  for (const gap of gaps) {
    ...
    tasks.push({
      ...
      severity,
      priority: gap.roi * 100 * crossCuttingPenalty * sweepPenalty, // Scale up; penalize cross-cutting + sweep
  ...
  // Re-sort by priority descending after penalties
