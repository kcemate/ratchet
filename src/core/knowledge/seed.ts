...
export const SEED_ENTRIES: KnowledgeEntry[] = [
...

  {
    ...
    description: 'Use a structured logger instead of console calls. Structured loggers ' +
...

  {
    ...
    pattern: 'catch\\s*\\([^)]*\\)\\s*\\{\\s*\\}',
    description: 'Empty catch blocks silently swallow errors. At minimum, log the ' +
      'caught error so failures are observable.',
    severity: 'high',
    language: 'typescript',
    examples: [
...

  {
    ...
    description: 'Async functions that lack try/catch let unhandled rejections crash ' +
...

  {
    ...
    description: 'Use toErrorMessage() from core/utils to extract error messages from ' +
...

  {
    ...
    pattern: 'console\\.log',
    description: 'console.log calls in production code add I/O overhead and create ' +
      'unstructured noise. Replace with structured logger or remove.',
    severity: 'low',
    language: 'typescript',
    examples: [
      "logger.debug({ value }, 'computed value');",
    ],
    antiPatterns: [
...

  {
    ...
    description: 'Awaiting inside a loop forces sequential execution. Use Promise.all ' +
...

  {
    ...
    description: 'Wildcard re-exports from barrel files and self-imports inflate bundle ' +
      'size and can cause circular dependency issues.',
    severity: 'medium',
    language: 'typescript',
    examples: [
      "export { loadKnowledge, query, getByCategory } from './store.js';",
    ],
    antiPatterns: [
...

  {
    ...
    description: 'Use Promise.allSettled when partial failures are acceptable, ' +
...

  {
    ...
    description: 'Functions longer than ~50 lines are hard to reason about. Extract ' +
...

  {
    ...
    pattern: '.{121,}',
    description: 'Lines over 120 characters reduce readability and cause horizontal ' +
...

  {
    ...
    description: 'TODO comments and commented-out code accumulate technical debt. ' +
      'Either implement the TODO immediately or delete it with a brief commit message.',
    severity: 'low',
    examples: [
      "// Removed legacy auth path — replaced by OAuth flow in PR #142",
    ],
    antiPatterns: [
      "// TODO: fix this later",
...

  {
    ...
    description: 'Repeated logic across files should be extracted into shared utility ' +
...

  {
    ...
    description: 'Unnamed numeric/string literals make intent opaque. Extract into ' +
    ...
    antiPatterns: [
      "if (retries >= 3) throw new Error('retry limit');\nawait sleep(5000);",
    ],
    references: [],
  },
...

  {
    ...
    description: 'Hardcoded secrets and API keys in source code are a critical security ' +
    ...
    antiPatterns: [
      "const apiKey = 'sk-1234abcdXYZ'; // hardcoded!",
      "const DB_PASS = 'super_secret_password';",
    ],
    references: [],
  },

  {
    ...
    description: 'Route handlers that use request data without validation are vulnerable ' +
...

  {
    ...
    description: 'Public routes without authentication or rate limiting are vulnerable ' +
    ...
    antiPatterns: [
      "router.post('/login', handler); // no auth, no rate limit",
    ],
    references: [],
  },

  {
    ...
    description: 'Using user-supplied input directly in file paths enables path traversal ' +
...

  {
    ...
    description: 'The `any` type disables TypeScript\'s safety guarantees. Use `unknown` ' +
...

  {
    ...
    description: 'TypeScript strict mode catches a class of bugs at compile time. ' +
...

  {
    ...
    subcategory: 'Any type count',
    pattern: '[a-zA-Z]!\\.',
    description: 'Non-null assertions (`!`) bypass null-checks and can cause runtime ' +
...

  {
    ...
    description: 'Typing catch variables as `any` re-introduces unsafety. Use `unknown` ' +
...

  {
    ...
    description: 'Async functions and error paths are the most commonly untested. ' +
...

  {
    ...
    description: 'Weak assertions like toBeDefined/toBeTruthy pass even for wrong values. ' +
...

  {
    ...
    description: 'Test boundary conditions: empty arrays, zero, null/undefined inputs, ' +
...

  {
    ...
    description: 'Tests that share mutable state are order-dependent and flaky. ' +
...

  {
    ...
    description: 'Every branch of conditional logic should have a test. Missing branch ' +
